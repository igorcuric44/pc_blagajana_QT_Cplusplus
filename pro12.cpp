...
class Unosn: public QWidget {
   
public:
    Unosn(QPushButton *tipka,QWidget *parent = 0);
    ~Unosn();
private:
QPushButton *tipka;
...
Unosn::Unosn(QPushButton *tipka,QWidget *parent): 
    QWidget(parent) 
{
    // Dummy UI
...
void Prozor::onClick6() {
  ...
  QTextStream out(stdout);
  out << "Button clicked sifrarnik" << endl;
  unosn= new Unosn(tipka6);
  unosn->setAttribute(Qt::WA_DeleteOnClose);
  unosn->show();
  unosn->activateWindow();
