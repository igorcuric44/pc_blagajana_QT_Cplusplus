...
public:
	printanje(QString cdx,QString ctx,QString racunx);
	~printanje();
private:
QPrinter printer;
QPainter painter;
...
printanje::printanje(QString cdx,QString ctx,QString racunx){
...
painter.drawText(0, 15, "0123456789");
painter.drawText(0, 30, cdx);
painter.drawText(0, 45, ctx);
painter.drawText(0, 60, racunx);
painter.end();
...
Blagajna::Blagajna(QPushButton *tipka,QWidget *blagajnax,QString p[],QWidget *parent)
  ...
  QString strxx=p[1]+" "+p[2]+"("+p[0]+")";
  
  label4y->setText(strxx);
...
QTextStream out(stdout);
  ...
  label3->setText(QString().sprintf("%0.2f",0.0));  
  suma=0.0;
  printanje print(cdx,ctx,racunx);
